...
\begin{abstract}
  ...
  seeds, \textsc{FADO} attains the best mean Accuracy, DP and EO against both \textsc{ARF} and \textsc{RFR} on both datasets, with all
  \textsc{Folktables} comparisons and all DP comparisons reaching paired Wilcoxon Holm-corrected $p<0.001$ and the remaining \textsc{COMPAS} comparisons at
  $p<0.01$ or below (except the \textsc{FADO}-vs-\textsc{RFR} accuracy comparison on \textsc{COMPAS}, which is not statistically distinguishable, $p=0.31$).
  \textsc{FADO} reduces demographic parity by $\sim$$61\%$ on \textsc{Folktables} and $\sim$$25\%$ on \textsc{COMPAS}. The
...
\keywords{fairness-aware online learning, concept drift, group fairness,
oblique decision forests, drift detection}
...
attribute-agnostic and does not need to be re-derived. For transparency we additionally report the \emph{marginal} demographic-parity and equalised-odds violations
along each attribute as diagnostics in Section~\ref{sec:results}, computed from the same prediction stream but with $\mathcal{A}_t$ restricted to a single attribute's
groups.
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.85\textwidth]{1846ea20-2f1d-415b-9b62-cb781adbf1f6.png}
...
\Description{Block diagram of the FADO prequential loop: an online error
stream feeds two ADWIN detectors (pre-warm and confirmation); the reaction
controller modulates the optimiser learning rate and the soft-routing
temperature; both quantities then enter the fairness-aware gradient step
of the Aranyani fair oblique decision forest.}
...
\multirow{4}{*}{\textsc{Folktables}}
 & \textbf{FADO} & $0.8033 \pm 0.0053$ & $\mathbf{0.0284 \pm 0.0041}^{**}$ & $0.0281 \pm 0.0027$ \\
 & Aranyani      & $\mathbf{0.8053 \pm 0.0049}$ & $0.0299 \pm 0.0032$ & $\mathbf{0.0257 \pm 0.0024}$ \\
 & ARF           & $0.7447 \pm 0.0104$ & $0.0722 \pm 0.0108$ & $0.0743 \pm 0.0153$ \\
 & RFR           & $0.7780 \pm 0.0047$ & $0.0733 \pm 0.0063$ & $0.0586 \pm 0.0065$ \\
\midrule
...
which carries no fairness mechanism, lands at $\mathrm{DP}=0.0722$ on the $2017$--$2018$ stream against \textsc{FADO}'s $0.0284$ and at $\mathrm{EO}=0.0743$
vs.\ \textsc{FADO}'s $0.0281$. \textsc{RFR}, which \emph{is} fairness-aware via sharpness-aware reweighting fitted on the $2015$ training data,
nonetheless drifts to $\mathrm{DP}=0.0733$ and $\mathrm{EO}=0.0586$ on the same test stream --- more than double \textsc{FADO}'s DP, despite enforcing
...
\textsc{Aranyani} baseline lands close to \textsc{FADO} on both datasets ($\mathrm{DP}=0.0299$ on \textsc{Folktables}, $0.0679$ on \textsc{COMPAS}) ---
...
\textsc{FADO} attains the best mean Accuracy, DP and EO against both external streaming baselines (\textsc{ARF} and \textsc{RFR}) on both datasets
(Table~\ref{tab:results}). On \textsc{Folktables} this manifests as $+5.9$pp Accuracy over \textsc{ARF} and $+2.5$pp over \textsc{RFR}, $\sim 61\%$ DP
reduction against each, and $\sim 62\%$/$\sim 52\%$ EO reduction; all six FADO-vs-\{\textsc{ARF},\textsc{RFR}\} comparisons reach Holm-corrected paired
Wilcoxon $p<0.001$. On \textsc{COMPAS} the gains are smaller in absolute terms ($+1.2$pp Accuracy over \textsc{ARF}, $\sim 24\%$ DP reduction over both)
and the significance is less uniform: DP wins against both at $p<0.001$, Accuracy against \textsc{ARF} at $p<0.001$, and EO against both at $p<0.01$;
Accuracy against \textsc{RFR} is in \textsc{FADO}'s favour by $+0.30$pp but not statistically distinguishable ($p=0.31$, paired Wilcoxon
Holm-corrected). Nine of the twelve FADO-vs-\{\textsc{ARF},\textsc{RFR}\} comparisons therefore land at $p<0.001$, two at $p<0.01$, and one is a tie. The
RQ2 answer against the accuracy-only and the fairness-without-adaptation baselines is consistent in direction across regimes: an adaptive in-processing
fairness-aware learner produces the highest mean on accuracy and the lowest mean on every fairness statistic, with the bulk of those wins being highly
significant.
...
controller is essentially tied with the bare \textsc{Aranyani} base learner: a $0.14$pp DP reduction in \textsc{FADO}'s favour ($p<0.01$), a $0.21$pp
accuracy and $0.25$pp EO deficit in \textsc{Aranyani}'s favour (both $p<0.001$), all well within seed-to-seed standard deviation. We
...
over $30$ seeds, \textsc{FADO} attains the best mean Accuracy, DP and EO against \textsc{ARF} and \textsc{RFR} on both datasets. Nine of the twelve
paired Wilcoxon comparisons land at Holm-corrected $p<0.001$ (every \textsc{Folktables} cell and every DP cell), two at $p<0.01$ (COMPAS EO against
\textsc{ARF} and \textsc{RFR}), and one --- COMPAS Accuracy against \textsc{RFR} --- is not statistically distinguishable ($+0.30$pp in \textsc{FADO}'s
favour, $p=0.31$). Demographic-parity reductions sit at $\sim 61\%$ on \textsc{Folktables} and in the $20$--$25\%$ range on \textsc{COMPAS}. The controller-isolation comparison against the bare \textsc{Aranyani} base learner is regime-dependent: on the abrupt-shift testbed the
