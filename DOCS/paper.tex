...
random seeds, \textsc{FADO} attains the best mean Accuracy, DP and EO against both \textsc{ARF} and \textsc{RFR} on both datasets, with all
...
\textsc{FADO} reduces demographic parity by $\sim$$61\%$/$\sim$$61\%$ on \textsc{Folktables} and $\sim$$24\%$/$\sim$$25\%$ on \textsc{COMPAS}. The
...
\begin{IEEEkeywords}
fairness-aware online learning, concept drift, drift detection, group fairness, oblique decision forests
\end{IEEEkeywords}
...
over $30$ random seeds, \textsc{FADO} attains the best mean Accuracy, DP and EO against \textsc{ARF} and \textsc{RFR} on both datasets. Nine of the twelve
