...
comparing against the \textsc{ARF} and \textsc{RFR} streaming baselines. \textsc{FADO} attains the highest prequential top-$1$ accuracy ($+3.84$ pp over
...
$42.4\%$ / $25.8\%$, all at Holm-corrected paired Wilcoxon $p<0.001$ aggregated over $30$ seeds. A complementary set of controlled concept-drift scenarios on
\textsc{COMPAS} is described as the testbed for the abrupt-shift regime; its multi-seed evaluation is in progress.
\end{abstract}
...
and the \textsc{Folktables} (ACS Income, CA) dataset. On \textsc{COMPAS} we inject four controlled synthetic concept-drift scenarios in which a row-level edit
on a categorical feature (\texttt{race}, \texttt{age\_cat}, or \texttt{c\_charge\_degree}) is paired with a deterministic flip of the recidivism label on the
edited rows, so that $P(y \mid x)$ shifts for the affected subpopulation rather than only the marginal $P(x)$. \textsc{Folktables} contributes a natural cross-year
shift between 2015 (training) and 2017--2018 (prequential test). \textsc{FADO} is compared against two streaming baselines covering the accuracy/fairness/adaptivity design
...
\item An empirical analysis, across four controlled concept-drift scenarios on \textsc{COMPAS} and one natural cross-year shift on \textsc{Folktables}, of how different forms of concept drift affect group fairness metrics over time,
and the corresponding performance results of \textsc{FADO} compared against the \textsc{ARF} and \textsc{RFR} baselines, demonstrating substantial improvements in fairness stability with competitive predictive performance.
\end{itemize}
...
\midrule
COMPAS & 7{,}214 & Two-year recidivism & Race (Caucasian vs.\ non-Caucasian) & 70/30 stratified train/test; four injected concept-drift scenarios on test stream \\
...
unmanipulated natural-drift testbed and reserve the four controlled scenarios below for \textsc{COMPAS}.

Each scenario combines (i) a row-level edit of a categorical feature in the test stream---\texttt{race}, \texttt{age\_cat}, or
\texttt{c\_charge\_degree}---with (ii) a deterministic flip of the \texttt{two\_year\_recid} label on the same edited rows. Pairing the categorical edit
with a per-subgroup label flip moves $P(y \mid x)$ as well as $P(x)$ for the affected subpopulation, yielding genuine
concept drift~\cite{DBLP:journals/corr/WebbHCNP15} rather than purely virtual covariate drift.

The \textsc{COMPAS} test stream is partitioned into three phases of equal protocol across scenarios: \emph{Warmup} ($0$--$20\%$, $\sim 430$ samples),
\emph{Drift} ($20\%$--$60\%$, $\sim 865$ samples), and \emph{Recovery} ($60\%$--$100\%$, $\sim 865$ samples). Warmup and recovery reflect the original
distribution; the scenario-specific edit-and-flip is applied only during the drift phase. The scenarios are:
\begin{itemize}
  \item \textbf{No drift}: the original test distribution is preserved throughout, providing a null reference for the controller's no-op behaviour;
  \item \textbf{Abrupt race}: during the drift phase, every recidivist African-American defendant has \texttt{race} relabeled Caucasian and \texttt{two\_year\_recid} flipped, modelling an abrupt and sustained inversion of the $\text{race}=\text{AA}\Rightarrow\text{recid}=1$ association the model has learned at training time;
  \item \textbf{Gradual race}: the same race-swap-and-label-flip event fires with linearly ramping probability $0 \to 1$ across the drift phase, modelling a slow but persistent shift in the protected-attribute conditional;
  \item \textbf{Age--race decoupling}: \texttt{age\_cat} is swapped between \texttt{Less than 25} and \texttt{Greater than 45} for the two race groups (AA $<$25 $\to$ $>$45 and Caucasian $>$45 $\to$ $<$25) during the drift phase, inverting the age-cat-by-race joint distribution, with \texttt{two\_year\_recid} flipped on the same rows;
  \item \textbf{Charge-degree race reversal}: \texttt{race} is swapped in stereotyped charge-degree combinations (African-American + Felony $\to$ Caucasian, Caucasian + Misdemeanor $\to$ African-American) during the drift phase, with \texttt{two\_year\_recid} flipped on the same rows. Unlike the race scenarios, this hits both directions of the protected attribute and affects all recidivism levels simultaneously.
\end{itemize}
The four non-baseline scenarios isolate, respectively, an abrupt label-correlated subpopulation shift, a gradual label-correlated subpopulation shift,
a narrow feature-co-occurrence inversion, and a wide-front bidirectional shift.
...
multipliers, and temperature schedule were tuned once on a pilot scenario using random search and held fixed across all other scenarios and datasets, to avoid per-scenario over-tuning of
the controller.
...
\label{sec:results}
We organise the prequential evaluation around the three research questions introduced in Section~I. 
Empirical evidence is drawn from the natural cross-year distribution shift between $2015$ and $2017$--$2018$ 
on \textsc{Folktables} (ACS Income, CA); the controlled concept-drift sweep on \textsc{COMPAS}
(Section~\ref{sec:method:scenarios}) is the testbed that will close the unaddressed half of each RQ and is the subject 
of a follow-up revision. All numbers are mean $\pm$ std over $30$ random seeds; \textbf{bold} marks the best mean per column.
...
\textsc{FADO} attains the highest prequential accuracy and the lowest fairness violation on every reported metric and against every baseline
(Table~\ref{tab:folktables}). Accuracy improves by up to roughly $4$ percentage points over the streaming baselines, demographic-parity violation is
reduced by up to $21\%$, and equalised odds---monitored but not directly optimised---is reduced by as much as $42\%$. All nine paired Wilcoxon comparisons
reach Holm-corrected significance at the $10^{-3}$ level with Cliff's $\delta = \pm 1$, and the seed-to-seed variance of demographic parity is more than
halved relative to the closest baseline (Brown--Forsythe test on equality of variances rejected at $p \approx 2 \times 10^{-14}$). The answer to RQ2 is
therefore affirmative on the natural-drift regime: keeping the node-level fairness regulariser continuously active under an adaptive controller wins on
accuracy, parity, and stability simultaneously, not at the expense of any of them. 

\subsection{RQ3: How Do Different Drift Types Affect Stability?}
...
the paper. This gives an affirmative answer to RQ2 on the gradual-shift regime, quantifies RQ1 (drift erodes the fairness guarantees of every non-adaptive
baseline we tested), and addresses one half of RQ3: gradual, low-amplitude shifts are tractable for a windowed in-processing learner with the controller
dormant. The complementary half of RQ3---abrupt, label-correlated subpopulation drift---is the regime the dual-ADWIN controller was designed for and is
addressed by the controlled concept-drift scenarios on \textsc{COMPAS} (Section~\ref{sec:method:scenarios}), whose multi-seed evaluation is in progress.
...
dynamics that the stream-averaged statistic hides. Second, we plan to extend the evaluation to multi-valued sensitive attributes and to equalised odds and
equal opportunity, and to run a full ablation of the controller's component design (label-noise guard, temperature modulation, LR pre-warm) to quantify
the contribution of each branch. Finally, hybridising \textsc{FADO} with smooth post-processing methods such as~\cite{Small_2024} during recovery windows
