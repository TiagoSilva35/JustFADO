...
detected drift, and anneals both back to baseline during recovery. We evaluate \textsc{FADO} under a prequential test-then-train protocol on \textsc{Folktables} ACS Income (CA; natural 2015 $\to$ 2017--2018 cross-year shift),
comparing against the \textsc{ARF} and \textsc{RFR} streaming baselines and against a controller-free \textsc{Aranyani} ablation that isolates the contribution of the drift-reaction controller. \textsc{FADO} attains the highest prequential top-$1$ accuracy ($+3.84$ pp over
\textsc{ARF}, $+3.30$ pp over \textsc{RFR}), reduces demographic-parity violation by $20.5\%$ / $13.5\%$, and reduces equalised-odds violation by
$42.4\%$ / $25.8\%$, all at Holm-corrected paired Wilcoxon $p<0.001$ aggregated over $30$ seeds. A $30$-seed evaluation on the \textsc{COMPAS}
controlled-drift testbed corroborates the finding on the abrupt-shift regime: \textsc{FADO} reduces demographic parity by $\sim 24\%$ over the non-fair
streaming baselines and by $\sim 10\%$ over a controller-free \textsc{Aranyani} ablation that shares the base learner and fairness loss (all
$p<0.001$), isolating the contribution of the reaction controller itself.
\end{abstract}
...
inject synthetic, parametrically controlled concept-drift perturbations on the test stream: this lets us isolate the dimension of distributional
change (label-correlated vs.\ wide-front, abrupt vs.\ gradual) and read the effect of each form of drift off the prequential curves under a known
ground truth. On \textsc{Folktables} (ACS Income, CA), training on $2015$ and evaluating prequentially on $2017$--$2018$ already provides a real,
...
flipped subset is a random half of the affected subpopulation rather than every row: at $p=1.0$ the flip is deterministic and the model can simply
learn the inverted rule, while at $p=0.5$ the flip is maximally entropic and the post-drift labelling is unlearnable. We chose $p=0.5$ because it produces
the steepest sustainable accuracy drop on \textsc{COMPAS}'s short stream---large enough to clear the ADWIN Hoeffding floor that scales as $1/\sqrt{m}$
(see Section~\ref{sec:method:hpo})---without making the drifted regime trivially separable in the opposite direction. Pairing the categorical edit with
the per-subgroup label flip moves $P(y \mid x)$ as well as $P(x)$ for the affected subpopulation, yielding genuine concept
drift~\cite{DBLP:journals/corr/WebbHCNP15} rather than purely virtual covariate drift.
...
across the drift phase with a $\mathrm{Bernoulli}(0.5)$ \texttt{two\_year\_recid} flip on the same rows. Three additional perturbations we
designed (a gradual race-swap variant, an age $\times$ race decoupling, a charge-degree race reversal) sit at or below ADWIN's Hoeffding floor on
\textsc{COMPAS}'s short stream under any controller setting that also keeps \emph{no\_drift} silent, so we defer them to a longer-stream evaluation.
...
\midrule
$\lambda$   & $10.0$              & $1.0$               & Fairness-penalty weight \\
...
The fairness weight $\lambda$ is the only knob with a non-geometric calibration. On \textsc{Folktables} it is selected by a compact NSGA-II procedure
(population $8$, $4$ generations) that jointly minimises prequential $1{-}\mathrm{Accuracy}$ and DP on a held-out portion of the $2015$ training
split, picking from the first Pareto front the candidate that minimises the normalised sum of objectives; this yields $\lambda=10$. On \textsc{COMPAS}
the held-out training subset is too small to support multi-generation NSGA-II reliably, so we use a one-dimensional pilot sweep over
$\lambda \in \{0.1, 1, 10\}$ measured against two diagnostic constraints: (i) on \emph{abrupt\_race} the controller must fire at least once on a
representative seed, and (ii) on \emph{no\_drift} the controller must remain dormant. At $\lambda=10$ the active regulariser dominates per-step updates
and absorbs the short-window accuracy excursion, so ADWIN never confirms drift on \emph{abrupt\_race}; at $\lambda=0.1$ the regulariser contribution
becomes too small for DP to track group statistics responsively; $\lambda=1$ is the largest value compatible with both diagnostics and is what we use.
...
excludes high-confidence-but-wrong predictions from the pre-warm error stream: in pilot runs this was the dominant false-positive mode --- single
samples where the forest is very confident but the noisy ground-truth flips --- and a $0.7$ threshold removed it without measurably affecting
true-positive detection of distributional change.
...
the fairness regulariser (Table~\ref{tab:hparams}). For \textsc{COMPAS}, results are aggregated per scenario across the three phases described in
Section~\ref{sec:method:scenarios}; for \textsc{Folktables}, results are aggregated over the full $2017$--$2018$ test stream.
...
$p$-values are corrected within each cell using the Holm--Bonferroni step-down procedure across all baselines. Effect sizes are reported as Cliff's $\delta$. For
the Folktables variance claim we use the Brown--Forsythe (Levene, median-centred) test on the per-seed DP values across all reported models. The procedure follows
the recommendations of Dem{\v{s}}ar's standard recipe for ML benchmark comparisons~\cite{demsar2006statistical}.

\section{Results and Discussion}
\label{sec:results}
Evidence is drawn from two complementary regimes: the natural cross-year shift on \textsc{Folktables} (Table~\ref{tab:folktables}, large/gradual) and the
controlled abrupt-shift testbed on \textsc{COMPAS} (Table~\ref{tab:compas}, short/synthetic). Both are reported as mean $\pm$ std over $30$ seeds.
Convergent findings across the two regimes are the central evidence for each research question.

\begin{table}[!htb]
\centering
\caption{\textsc{Folktables} cross-year results. \textbf{Bold} = best mean; $^{***}$ = paired Wilcoxon $p{<}0.001$ on the \textsc{FADO} row vs each baseline.}
...
\midrule
\TODO{Update the results}\textbf{FADO} & $\mathbf{0.8217 \pm 0.0025}^{***}$ & $\mathbf{0.0635 \pm 0.0019}^{***}$ & $\mathbf{0.0412 \pm 0.0020}^{***}$ \\
Aranyani      & \TODO{acc}                       & \TODO{dp}                        & \TODO{eo} \\
...
\begin{table}[!htb]
\centering
\caption{\textsc{COMPAS} results, averaged across the two active scenarios. \textbf{Bold} = best mean per column. Superscripts on the \textsc{FADO} row = Holm-corrected paired Wilcoxon vs \emph{every} baseline at the indicated level ($^{**}p{<}0.01$, $^{***}p{<}0.001$). The accuracy column carries no marker because the per-baseline comparisons are mixed in direction; they are dissected in the text.}
\label{tab:compas}
\setlength{\tabcolsep}{3pt}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lccc}
...
The Folktables cross-year shift inflates the fairness violation of every non-adaptive baseline (Table~\ref{tab:folktables}). The static fairness reweighting
of \textsc{RFR}, fitted on $2015$ data, fails to maintain its training-time guarantees on the $2017$--$2018$ stream; \textsc{ARF}, with no fairness
mechanism at all, fares worse still. Relative to \textsc{FADO}, the baselines run up to a quarter higher on demographic parity and up to roughly three
quarters higher on equalised odds. The effect is consistent across both fairness statistics and across both baselines, and shows that drift erodes the
fairness guarantees obtained at training time even when the corresponding predictive-accuracy gap is modest.

The same pattern reproduces on the controlled-drift testbed (Table~\ref{tab:compas}). The non-fair baselines sit at $\mathrm{DP}\approx 0.081$ against
\textsc{FADO}'s $0.061$ ($\sim 24\%$ relative reduction, paired Wilcoxon $p<0.001$ in both cases). The controller-free \textsc{Aranyani} baseline sits at
$\mathrm{DP}=0.0679$, midway between \textsc{FADO} and the non-fair baselines: even with the node-level fairness regulariser continuously active,
removing the reaction controller leaves a $\sim 10\%$ DP deficit that the abrupt shift opens up ($p<0.001$). The directional consistency across the
natural gradual shift (\textsc{Folktables}) and the synthetic abrupt shift (\textsc{COMPAS}) is the central RQ1 finding: drift erodes fairness against
unconstrained streaming ensembles \emph{and} against a static fairness-aware learner.

\subsection{RQ2: Can Adaptive Mechanisms Preserve Fairness While Maintaining Utility?}
\label{sec:results:rq2}
\textsc{FADO} attains the highest prequential accuracy and the lowest fairness violation on every reported metric and against every baseline on the
\textsc{Folktables} natural-drift regime (Table~\ref{tab:folktables}). Accuracy improves by up to roughly $4$ percentage points over the streaming baselines,
demographic-parity violation is reduced by up to $21\%$, and equalised odds---monitored but not directly optimised---is reduced by as much as $42\%$. All nine
paired Wilcoxon comparisons reach Holm-corrected significance at the $10^{-3}$ level with Cliff's $\delta = \pm 1$, and the seed-to-seed variance of demographic
parity is more than halved relative to the closest baseline (Brown--Forsythe test on equality of variances rejected at $p \approx 2 \times 10^{-14}$). The
answer to RQ2 is therefore affirmative on the natural-drift regime: keeping the node-level fairness regulariser continuously active under an adaptive
controller wins on accuracy, parity, and stability simultaneously, not at the expense of any of them.

The \textsc{COMPAS} testbed (Table~\ref{tab:compas}) refines this finding by isolating the contribution of the reaction controller itself. \textsc{FADO}
and the controller-free \textsc{Aranyani} baseline share the identical base learner, fairness loss, rolling-window statistics, and seed-pinned data
ordering, so the gap between the two rows is attributable solely to the dual-ADWIN controller. On this comparison, \textsc{FADO} reduces DP by $0.68$pp
($\sim 10\%$, $p<0.001$) and EO by $0.56$pp ($\sim 6\%$, $p<0.001$) at a $0.17$pp accuracy cost ($p<0.001$ in the opposite direction). Against the
non-fair streaming baselines the DP reduction scales up to $\sim 24\%$ (both $p<0.001$). The accuracy result deserves a short clarification: the
paired Wilcoxon test is sensitive to the \emph{sign} of the per-seed difference rather than to its magnitude, so a $0.17$pp deficit that is consistent
across $n=30$ seeds is enough to reject $H_0$ even though the practical effect is well within the standard deviation of either model. The mechanism is
expected --- the controller spikes the learning rate on every confirmed drift event and transiently elevates the training-time loss, which shows up as a
small but reproducible per-seed accuracy reduction relative to the controller-free baseline. The relevant question is whether the trade is
\emph{favourable}: each $1$pp of accuracy traded yields $\approx 4$pp of DP improvement and $\approx 3$pp of EO improvement against the controller-free
ablation, all at $p<0.001$ in their respective directions. RQ2 is therefore answered affirmatively in both regimes: against an accuracy-oriented
baseline (\textsc{ARF}) the controller wins on every axis; against the fairness-aware baselines (\textsc{Aranyani}, \textsc{RFR}) it converts a small
accuracy concession into substantially improved fairness stability.

\subsection{On the Two Statistical Tests}
\label{sec:results:tests}
We report the paired Wilcoxon signed-rank test as the primary statistic, following the prequential-benchmark recommendations of Dem\v{s}ar~\cite{demsar2006statistical}:
it makes no parametric assumption about the per-seed metric distribution, and it exploits the within-seed pairing produced by the shared data ordering
(Section~\ref{sec:method:baselines}), which is precisely the structure that makes the Welch two-sample $t$-test discard relevant information. The two
tests can therefore disagree in instructive ways. The clearest example in our data is the \textsc{FADO}-vs-\textsc{Aranyani} accuracy comparison on
\textsc{COMPAS}: Welch is non-significant ($p=0.66$) because the per-seed difference is small relative to the cross-seed standard deviation of either
model in isolation, while paired Wilcoxon is highly significant ($p<0.001$) because that same difference is consistent in sign across $28$ of $30$
paired seeds. Both numbers are correct; they answer different questions. Welch asks whether the two models' marginal distributions differ; paired
Wilcoxon asks whether the controller systematically shifts the per-seed outcome in one direction. The second is the question RQ2 requires and is the one
...
Empirically, on the natural cross-year shift in Folktables, \textsc{FADO} simultaneously improves accuracy ($+3.84$ pp over \textsc{ARF}, $+3.30$ pp over
\textsc{RFR}) and demographic parity ($-20.5\%$, $-13.5\%$) with substantially lower seed-to-seed variance---the cleanest evidence for the central claim of
the paper. The controlled abrupt-shift testbed on \textsc{COMPAS} (Section~\ref{sec:method:scenarios}) reproduces the directional finding at $n=30$ seeds:
\textsc{FADO} reduces DP by $\sim 24\%$ over the non-fair streaming baselines and by $\sim 10\%$ over a controller-free \textsc{Aranyani} ablation that
shares the base learner and fairness loss (all $p<0.001$, paired Wilcoxon with Holm correction), at a $0.17$pp accuracy trade-off. Taken
together the two evaluations quantify RQ1 (drift erodes the fairness guarantees of every non-adaptive baseline we tested, on both natural-gradual and
synthetic-abrupt regimes) and give an affirmative answer to RQ2: keeping the node-level fairness regulariser continuously active under an adaptive
controller wins on parity and stability simultaneously, and on accuracy when an accuracy-oriented streaming baseline (\textsc{ARF}) is in the comparison.
...
dynamics that the stream-averaged statistic hides. Second, we plan to extend the evaluation to multi-valued sensitive attributes and to wire the already-implemented equalised-odds (and equal-opportunity)
regulariser into the gradient update once the per-step cost is amortised, allowing direct optimisation rather than diagnostic-only monitoring; we also plan a
full ablation of the controller's component design (label-noise guard, temperature modulation, LR pre-warm) to quantify the contribution of each branch. Finally, hybridising \textsc{FADO} with smooth post-processing methods such as~\cite{Small_2024} during recovery windows
is a promising direction, since the two mechanisms address fairness at orthogonal layers of the prediction pipeline.
